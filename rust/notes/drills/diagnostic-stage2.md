...
| b16 | 次のイベント | **BTreeMap** 範囲クエリ `range(x..).next()` | 🟢 | `range(x..).next()`=x以上で最小キー。結果は`Option`なので`or_insert`でなく`map/unwrap_or`。rangeはキー専用。→[[../problems/drill/drill-b16]] |
| b17 | 値ごとの個数(昇順) | **BTreeMap** で自動昇順集計 | 🟢 自力AC | 集計してそのまま回すだけ（sort不要）。`entry().or_default()+=1`はまだチートシート要だが3問目で定着中。→[[../problems/drill/drill-b17]] |
| b18 | 最高得点者 | 参照/deref 総合（`Vec<(String,i64)>`） | 🟢 | BTreeMap使おうとして詰まる→rangeはキー専用で値検索不可。Vec素直に回すのが正解。min vs sortの計算量(b3再確認)、String clone。→[[../problems/drill/drill-b18]] |
