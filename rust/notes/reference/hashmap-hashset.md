...
（`or_insert(0)` は仕組みを見せるため。**実際は `or_default()` でOK** → `*count.entry(key).or_default() += 1;`。下の entry API 参照）
...
> **まずは `or_default()` 一本でOK。** カウントもグループ化も両方これで書ける:
> ```rust
> *count.entry(key).or_default() += 1;          // カウント（規定値 0）
> groups.entry(key).or_default().push(value);   // グループ化（規定値 空Vec）
> ```
> `or_insert(値)` を出すのは **初期値が 0/空 以外** のときだけ（例：最小値追跡で `or_insert(i64::MAX)`）。
> `i64::MAX`/`i64::MIN` は Rust 組み込みの定数（∞/-∞ のかわり）。**最小を探すなら MAX から、最大を探すなら MIN から**始めると、最初の `.min()`/`.max()` で必ず本物の値に置き換わる。`and_modify` は有/無で違う処理をしたい時だけ。
