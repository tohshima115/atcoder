...
### ★ 構築もクエリも「同じ包除の4項」（drill-s1 の気づき）

2次元累積和は **作るときも引くときも、符号だけ違う同じ4項**になっている。ここに気づくと一気に腑に落ちる。

```text
構築:  pre[i+1][j+1] = grid + pre[上] + pre[左] − pre[左上]    （+ + + −）
クエリ:    sum       =        pre[全] − pre[上] − pre[左] + pre[左上]   （+ − − +）
```

`|A∪B| = |A| + |B| − |A∩B|` のベン図と構造が同じ（引きすぎた左上の領域を足し戻す）。

#### 1パス（上の漸化式そのもの）vs 2パス

| | 1パス漸化式 | 2パス（横に積む→縦に積む） |
|---|---|---|
| ループ | 二重ループ1個 | 二重ループ2個 |
| 計算量 | O(HW) | O(HW)（**全く同じ**） |
| 長所 | 「構築=クエリ」の対称が脳に焼ける | 各パスが単純でバグらせにくい |

```rust
// 2パス版（横プレフィックス → 縦に積む）。1パスが不安なときの安全策
for i in 1..=h { for j in 1..=w { pre[i][j] = pre[i][j-1] + g[i-1][j-1]; } }   // 横
for j in 1..=w { for i in 1..=h { pre[i][j] += pre[i-1][j]; } }                // 縦
```

計算量は同じ（O(HW) が下限：全マス1回は読むので、これより速くは作れない）。**本番は手が覚えてる方でよい。**
