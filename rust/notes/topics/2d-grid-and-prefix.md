...
## ⓪ グリッドの表現は「操作」で選ぶ（→ [[abc322d]]）

グリッド＝いつも2次元配列、ではない。**やりたい操作**で持ち方を変えると楽になる。

| 表現 | 得意なこと | 使う場面 |
|---|---|---|
| 2次元配列 `Vec<Vec<char>>` | 隣接を見る・書き換える | BFS/DFS・迷路・塗りつぶし（大多数はこれ、以下①〜） |
| 座標リスト `Vec<(i,j)>` | 回転・移動・点集合 | 「図形を動かす」問題。回転は `(r,c)→(c,-r)` |
| ビットマスク `u16` 等 | 重なり判定・合体・状態圧縮 | マスが少ない（〜20）時の集合演算（→ [[bit-operations]]） |

「隣を辿る→配列 / 形を動かす→座標 / 重なり・集合→ビット」。abc322 D は座標リスト（回転用）とビットマスク（重なり判定用）を併用した。
